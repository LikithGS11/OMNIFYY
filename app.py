...
def get_calendar_days():
    # For demo, generate 7 days starting today
...
def get_classes():
    # Fetch classes from DB and add dummy trainer photos and ratings for demo
    conn = sqlite3.connect("fitness.db")
    cursor = conn.cursor()
    cursor.execute("SELECT name, datetime, instructor FROM classes")
    rows = cursor.fetchall()
    conn.close()

    classes = []
    photos = {
        "Dayana Mam": "trainer1.jpg",
        "Vayun Sir": "trainer2.jpg",
        "Krishna Sir": "trainer3.jpg",
        "Guna Sir": "trainer4.jpg",
        "Likith Sir": "trainer5.jpg"
    }
    ratings = {
        "Dayana Mam": 5,
        "Vayun Sir": 4,
    ...
    for row in rows:
        name, dt_str, instructor = row
        dt = datetime.fromisoformat(dt_str)
        time_slot = dt.strftime("%I:%M %p")
        classes.append({
            "name": name,
            "time_slot": time_slot,
            "instructor": instructor,
            "trainer_photo": photos.get(instructor, "trainer_default.jpg"),
            "rating": ratings.get(instructor, 5)
        })
    return classes
...
def home():
    ...
    # Render the homepage with calendar and classes
